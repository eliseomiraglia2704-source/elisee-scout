...
(function () {
  ...
  function hideOthers() {
    ['es-cd', 'es-dsd', 'es-prd', 'es-vd', 'es-fd', 'es-mad', 'es-md', 'es-od', 'es-tmd', 'es-gk', 'es-atd', 'es-yg', 'es-dg', 'es-ag', 'es-pr'].forEach(function (id) {
  ...
  function openMkEditModal(user) {
    ...
    backdrop.innerHTML = '<div class="es-edit-modal">' +
      ...
      '<h2><span>✏️</span> Modifica Anagrafica Marketing & Commerciale</h2>' +
    ...
    backdrop.querySelector('.es-edit-btn-save').addEventListener('click', function () {
      var n = document.getElementById('es-mk-nome').value.trim();
      var c = document.getElementById('es-mk-cognome').value.trim();
      var clb = document.getElementById('es-mk-club').value.trim();
      var bio = document.getElementById('es-mk-bio').value.trim();
      ...
      if (typeof window.showToast === 'function') {
        window.showToast('Anagrafica Marketing & Commerciale salvata con successo!', 'success');
      }
      render(user);
    });
  }
  ...
  function render(user) {
    ...
    host.classList.remove('es-pd-on', 'es-ds-on', 'es-pres-on', 'es-vice-on', 'es-fisio-on', 'es-ma-on', 'es-med-on', 'es-obs-on', 'es-tm-on', 'es-gk-on', 'es-at-on', 'es-yg-on', 'es-dg-on', 'es-ag-on', 'es-pr-on');
    if (group) {
      group.classList.add('is-mk-dash');
      group.classList.remove('is-coach-dash', 'is-ds-dash', 'is-pres-dash', 'is-vice-dash', 'is-fisio-dash', 'is-ma-dash', 'is-med-dash', 'is-obs-dash', 'is-tm-dash', 'is-gk-dash', 'is-at-dash', 'is-yg-dash', 'is-dg-dash', 'is-ag-dash', 'is-pr-dash');
    }
    bind(host);
  }
