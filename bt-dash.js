...
(function () {
  ...
  function hideOthers() {
    if (typeof window.unmountAllRoleDashboards === "function") {
      window.unmountAllRoleDashboards('es-bt');
    }
  }
  ...
  function html(user) {
    return window.EliseeDashReal.shell({
      user: user,
      title: 'Elisee Scout — Dashboard Biglietteria',
      roleLabel: 'Responsabile biglietteria',
      attr: 'bt',
      extraRail: '',
      radarTitle: 'Quadro affluenza',
      workTitle: 'Vendite',
      workEmpty: 'Nessuna vendita registrata.',
      registroTitle: 'Registro biglietti',
      registroHeaders: ['Partita', 'Settore', 'Incasso']
    });
  }
  ...
  function openBtEditModal(user) {
    ...
    backdrop.innerHTML = '<div class="es-edit-modal">' +
      ...
      '<div class="es-edit-field"><label>Nome</label><input id="es-bt-nome" value="' + esc(user.nome || '') + '"></div>' +
      '<div class="es-edit-field"><label>Cognome</label><input id="es-bt-cognome" value="' + esc(user.cognome || '') + '"></div>' +
      '<div class="es-edit-field"><label>Ruolo Ufficiale</label><input id="es-bt-role" value="Resp. Biglietteria / SLO" readonly></div>' +
      '<div class="es-edit-field"><label>Club / Organizzazione</label><input id="es-bt-club" value="' + esc(user.squadra || user.club || '') + '"></div>' +
      '<div class="es-edit-field full"><label>Bio &amp; Note Operative</label><textarea id="es-bt-bio" rows="3">' + esc(user.bio || '') + '</textarea></div>' +
      '</div>' +
