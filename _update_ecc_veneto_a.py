# -*- coding: utf-8 -*-
"""Organico ufficiale Eccellenza Veneto Girone A 2026/27 + loghi Tuttocampo."""
...
SHOTS = [
    {
        "file": "image-12b891fd-b31a-4d32-afe2-4313e57defdb.png",
        "league": "ECCELLENZA · VENETO · GIRONE A",
        "gid": "veneto-a",
        "title": "Veneto · Girone A",
        "area": "Eccellenza · organico ufficiale 2026/27 (16 squadre)",
        "pairs": [
            (("Adriese", "Adria"), ("Montecchio Maggiore", "Montecchio Maggiore")),
            (("Ambrosiana", "Sant'Ambrogio di Valpolicella"), ("Montorio FC", "Montorio")),
            (("AQS Borgo Veneto", "Borgo Veneto"), ("Oppeano", "Oppeano")),
            (("Berton Bolzano Vicentino", "Bolzano Vicentino"), ("Piovese", "Piove di Sacco")),
            (("Castelnuovo del Garda", "Castelnuovo del Garda"), ("Pozzonovo", "Pozzonovo")),
            (("Chiampo", "Chiampo"), ("Vigasio", "Vigasio")),
            (("Limena", "Limena"), ("Villafranca Veronese", "Villafranca di Verona")),
            (("Mestrino United", "Mestrino"), ("Villafranchese", "Villafranca Padovana")),
        ],
    },
]
...
def row_centers(bgr: np.ndarray, n_rows: int) -> list[int]:
    h, w = bgr.shape[:2]
    # 8-row Toscana A table sits lower than Molise; variance peaks ~0.331–0.837
    first = int(h * 0.338)
    last = int(h * 0.837)
    if n_rows <= 1:
        return [first]
...
def main():
    ...
    for shot in SHOTS:
        ...
        for pair in shot["pairs"]:
            names.append(pair[0][0])
            cities.append(pair[0][1])
            if len(pair) > 1:
                names.append(pair[1][0])
                cities.append(pair[1][1])
        expected = len(names)
