...
(function () {
  ...
  function html(user) {
    return window.EliseeDashReal.shell({
      user: user,
      title: 'Elisee Scout — Dashboard Preparatore Portieri',
      roleLabel: 'Preparatore dei portieri',
      attr: 'gk',
      extraRail: '',
      radarTitle: 'Quadro tecnico portieri',
      workTitle: 'Sessioni',
      workEmpty: 'Nessuna sessione registrata.',
      registroTitle: 'Registro sessioni',
      registroHeaders: ['Data', 'Tema', 'Esito']
    });
  }
  ...
  function openGkEditModal(user) {
    ...
    backdrop.innerHTML = '<div class="es-edit-modal">' +
      ...
      '<div class="es-edit-field full"><label>Bio &amp; Note Operative</label><textarea id="es-gk-bio" rows="3">' + esc(user.bio || '') + '</textarea></div>' +
      '</div>' +
