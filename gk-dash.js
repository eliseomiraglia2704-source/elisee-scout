/* Dashboard Preparatore dei Portieri */
(function () {
  var AXES = [
    'Preparazione Tecnica', 'Reattività Allenamenti', 'Gestione Uscite', 'Lavoro sui Piedi',
    'Analisi Avversari', 'Analisi Video', 'Prevenzione Infortuni Specifici', 'Sviluppo Vivaio'
  ];
  var V2025 = [87, 90, 94, 88, 87, 91, 91, 94];
  ...
  function isGk(u) {
    ...
    return /preparatore dei portieri|preparatore portieri/.test(blob);
  }
  function gkName(u) {
    return [u.nome, u.cognome].filter(Boolean).join(' ').trim() || u.username || 'Preparatore portieri';
  }
  function initials(name) {
    var p = String(name || 'PP').trim().split(/\s+/);
    return ((p[0] || 'P').charAt(0) + (p[1] || p[0] || 'P').charAt(0)).toUpperCase();
  }
  function photoOf(u) {
    try {
  ...
  function radarSvg() {
    ...
    var html = '<svg viewBox="0 0 440 430" role="img" aria-label="Analisi attività tecnica portieri">';
  ...
  function hideOthers() {
    ['es-cd', 'es-dsd', 'es-prd', 'es-vd', 'es-fd', 'es-mad', 'es-md', 'es-od', 'es-tmd', 'es-atd', 'es-yg'].forEach(function (id) {
      var el = document.getElementById(id);
      if (el) el.hidden = true;
    });
    var g = document.getElementById('user-dossier-view-group');
    if (g) g.classList.remove('is-coach-dash', 'is-ds-dash', 'is-pres-dash', 'is-vice-dash', 'is-fisio-dash', 'is-ma-dash', 'is-med-dash', 'is-obs-dash', 'is-tm-dash', 'is-at-dash', 'is-yg-dash');
  }
  ...
  function html(user) {
    ...
    return '<aside class="es-pd-rail">' +
      ...
      '<button type="button" data-gk="album" title="Album">' + ico('<rect width="18" height="18" x="3" y="3" rx="2" ry="2"/><circle cx="9" cy="9" r="2"/><path d="m21 15-3.086-3.086a2 2 0 0 0-2.828 0L6 21"/>') + '</button>' +
      '<button type="button" data-gk="msgs" title="Messaggi">' + ico('<rect width="20" height="16" x="2" y="4" rx="2"/><path d="m22 7-8.97 5.7a1.94 1.94 0 0 1-2.06 0L2 7"/>') + '</button>' +
      '<button type="button" class="es-pd-rail-end" data-gk="edit" title="Anagrafica">' + ico('<path d="M12.22 2h-.44a2 2 0 0 0-2 2v.18a2 2 0 0 1-1 1.73l-.43.25a2 2 0 0 1-2 0l-.15-.08a2 2 0 0 0-2.73.73l-.22.38a2 2 0 0 0 .73 2.73l.15.1a2 2 0 0 1 1 1.72v.51a2 2 0 0 1-1 1.74l-.15.09a2 2 0 0 0-.73 2.73l.22.38a2 2 0 0 0 2.73.73l.15-.08a2 2 0 0 1 2 0l.43.25a2 2 0 0 1 1 1.73V20a2 2 0 0 0 2 2h.44a2 2 0 0 0 2-2v-.18a2 2 0 0 1 1-1.73l.43-.25a2 2 0 0 1 2 0l.15.08a2 2 0 0 0 2.73-.73l.22-.39a2 2 0 0 0-.73-2.73l-.15-.08a2 2 0 0 1-1-1.74v-.5a2 2 0 0 1 1-1.74l.15-.09a2 2 0 0 0 .73-2.73l-.22-.38a2 2 0 0 0-2.73-.73l-.15.08a2 2 0 0 1-2 0l-.43-.25a2 2 0 0 1-1-1.73V4a2 2 0 0 0-2-2z"/><circle cx="12" cy="12" r="3"/>') + '</button>' +
      '</aside><div class="es-pd-body">' +
      '<div class="es-pd-head"><h1>Elisee Scout — Dashboard Preparatore Portieri</h1>' +
      '<strong>Preparatore portieri: ' + esc(name.toUpperCase()) + '</strong></div>' +
      '<div class="es-pd-grid">' +

      '<section class="es-pd-card es-pd-indice"><h2>Profilo Preparatore Portieri</h2>' +
      ...
      '<div class="es-pd-metric"><span>Gestione allenamenti</span><b>85%</b></div>' +
      '<div class="es-pd-metric"><span>Analisi video portieri</span><b>88%</b></div>' +
      '<div class="es-pd-metric"><span>Rapporto con lo staff</span><b>94%</b></div>' +
      '<div class="es-pd-metric"><span>Sviluppo giovani portieri</span><b>91%</b></div></section>' +

      '<section class="es-pd-card es-pd-radar">' +
      '<div class="es-pd-radar-tools"><span>Seleziona dati radar</span><span>Analisi attività tecnica portieri</span><div class="es-pd-legend-pills"><span class="es-pd-pill-legend" style="color:#38bdf8"><i style="background:#38bdf8"></i> 2025 (Stagione Attuale)</span><span class="es-pd-pill-legend" style="color:#94a3b8"><i style="background:#64748b"></i> 2023 (Benchmark Storico)</span></div></div>' +
      radarSvg() + '</section>' +

      '<section class="es-pd-card es-pd-comply"><h2>Verifica &amp; Compliance staff tecnico</h2>' +
      '<div class="es-pd-ok"><span>Patentino UEFA Preparatore Portieri</span><b>100%</b></div>' +
      ...
      '<section class="es-pd-card es-pd-storico"><h2>Andamento preparazione portieri</h2>' +
      '<div class="es-pd-sparks">' +
      '<figure>' + spark([32, 40, 48, 55, 64, 74, 86], '#38bdf8') + '<figcaption>Sessioni specifiche</figcaption></figure>' +
      '<figure>' + spark([28, 36, 44, 52, 60, 72, 84], '#4ade80') + '<figcaption>Parate allenate</figcaption></figure>' +
      '<figure>' + spark([40, 48, 52, 58, 66, 74, 82], '#facc15') + '<figcaption>Progressi portieri</figcaption></figure>' +
      ...
      '<p class="es-gk-grade">8,5/10 <small>+5,5%</small></p>' +
      '<div class="es-pd-mrow"><span>Valutazione allenatore capo</span><b>Ottima</b></div>' +
      '<div class="es-pd-mrow"><span>Trend progressi portieri</span><b>Crescente</b></div>' +
      ...
      '<section class="es-pd-card es-pd-registro"><h2>Registro sessioni portieri</h2>' +
      '<table class="es-pd-table"><thead><tr><th>Data</th><th>Portiere</th><th>Focus</th><th>Valutazione</th></tr></thead><tbody>' +
      '<tr><td>15/07/2026</td><td>Rossi</td><td>Uscite basse</td><td>Eccellente <i class="es-pd-dot g"></i></td></tr>' +
      '<tr><td>27/07/2026</td><td>Rossi</td><td>Uscite basse</td><td>Eccellente <i class="es-pd-dot g"></i></td></tr>' +
      '<tr><td>16/09/2026</td><td>Rossi</td><td>Uscite basse</td><td>Eccellente <i class="es-pd-dot g"></i></td></tr>' +
      '<tr><td>16/09/2026</td><td>Rossi</td><td>Uscite basse</td><td>Eccellente <i class="es-pd-dot g"></i></td></tr>' +
      '<tr><td>16/11/2026</td><td>Rossi</td><td>Uscite basse</td><td>Eccellente <i class="es-pd-dot g"></i></td></tr>' +
      '<tr><td>16/09/2026</td><td>Rossi</td><td>Lavoro sui piedi</td><td>In corso <i class="es-pd-dot y"></i></td></tr>' +
      '</tbody></table></section>' +

      '<section class="es-pd-card es-pd-trend"><h2>2023 vs 2024 vs 2025</h2>' +
      trendSvg() +
      '<button type="button" class="es-pd-edit" data-gk="edit">Modifica anagrafica</button>' +
      '</section>' +
      '</div></div>';
  }
  ...
  function openGkEditModal(user) {
    ...
    backdrop.innerHTML = '<div class="es-edit-modal">' +
      ...
      '<div class="es-edit-field"><label>Nome</label><input id="es-gk-nome" value="' + esc(user.nome || 'Eliseo') + '"></div>' +
      '<div class="es-edit-field"><label>Cognome</label><input id="es-gk-cognome" value="' + esc(user.cognome || 'Miraglia') + '"></div>' +
      '<div class="es-edit-field"><label>Ruolo Ufficiale</label><input id="es-gk-role" value="Preparatore dei Portieri" readonly></div>' +
      '<div class="es-edit-field"><label>Club / Organizzazione</label><input id="es-gk-club" value="' + esc(user.squadra || user.club || 'Notaresco Calcio') + '"></div>' +
      '<div class="es-edit-field full"><label>Bio &amp; Note Operative</label><textarea id="es-gk-bio" rows="3">' + esc(user.bio || 'Profilo accreditato e verificato su Elisee Scout per la stagione 2025/2026.') + '</textarea></div>' +
      '</div>' +
    ...
    backdrop.querySelector('.es-edit-btn-save').addEventListener('click', function () {
      ...
      if (typeof window.showToast === 'function') {
        window.showToast('Anagrafica Preparatore dei Portieri salvata con successo!', 'success');
      }
      render(user);
    });
  }
  ...
  function bind(host) {
    ...
    host.addEventListener('click', function (e) {
      var b = e.target.closest('[data-gk]');
      if (!b) return;
