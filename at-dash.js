/* Dashboard Preparatore Atletico */
(function () {
  var AXES = [
  ...
  var V2025 = [87, 92, 90, 94, 85, 93, 87, 91];
  var V2023 = [74, 78, 76, 80, 70, 79, 73, 77];
  ...
  function isAt(u) {
    ...
    return /preparatore atletico/.test(blob);
  }
  function atName(u) {
    return [u.nome, u.cognome].filter(Boolean).join(' ').trim() || u.username || 'Preparatore atletico';
  }
  function initials(name) {
    var p = String(name || 'PA').trim().split(/\s+/);
    return ((p[0] || 'P').charAt(0) + (p[1] || p[0] || 'A').charAt(0)).toUpperCase();
  }
  function photoOf(u) {
    try {
  ...
  function radarSvg() {
    ...
    var html = '<svg viewBox="0 0 440 430" role="img" aria-label="Analisi attività di preparazione atletica">';
  ...
  function hideOthers() {
    ['es-cd', 'es-dsd', 'es-prd', 'es-vd', 'es-fd', 'es-mad', 'es-md', 'es-od', 'es-tmd', 'es-gk', 'es-yg'].forEach(function (id) {
      var el = document.getElementById(id);
      if (el) el.hidden = true;
    });
    var g = document.getElementById('user-dossier-view-group');
    if (g) g.classList.remove('is-coach-dash', 'is-ds-dash', 'is-pres-dash', 'is-vice-dash', 'is-fisio-dash', 'is-ma-dash', 'is-med-dash', 'is-obs-dash', 'is-tm-dash', 'is-gk-dash', 'is-yg-dash');
  }
  ...
  function html(user) {
    ...
    return '<aside class="es-pd-rail">' +
      ...
      '<button type="button" data-at="album" title="Album">' + ico('<rect width="18" height="18" x="3" y="3" rx="2" ry="2"/><circle cx="9" cy="9" r="2"/><path d="m21 15-3.086-3.086a2 2 0 0 0-2.828 0L6 21"/>') + '</button>' +
      '<button type="button" data-at="msgs" title="Messaggi">' + ico('<rect width="20" height="16" x="2" y="4" rx="2"/><path d="m22 7-8.97 5.7a1.94 1.94 0 0 1-2.06 0L2 7"/>') + '</button>' +
      '<button type="button" class="es-pd-rail-end" data-at="edit" title="Anagrafica">' + ico('<path d="M12.22 2h-.44a2 2 0 0 0-2 2v.18a2 2 0 0 1-1 1.73l-.43.25a2 2 0 0 1-2 0l-.15-.08a2 2 0 0 0-2.73.73l-.22.38a2 2 0 0 0 .73 2.73l.15.1a2 2 0 0 1 1 1.72v.51a2 2 0 0 1-1 1.74l-.15.09a2 2 0 0 0-.73 2.73l.22.38a2 2 0 0 0 2.73.73l.15-.08a2 2 0 0 1 2 0l.43.25a2 2 0 0 1 1 1.73V20a2 2 0 0 0 2 2h.44a2 2 0 0 0 2-2v-.18a2 2 0 0 1 1-1.73l.43-.25a2 2 0 0 1 2 0l.15.08a2 2 0 0 0 2.73-.73l.22-.39a2 2 0 0 0-.73-2.73l-.15-.08a2 2 0 0 1-1-1.74v-.5a2 2 0 0 1 1-1.74l.15-.09a2 2 0 0 0 .73-2.73l-.22-.38a2 2 0 0 0-2.73-.73l-.15.08a2 2 0 0 1-2 0l-.43-.25a2 2 0 0 1-1-1.73V4a2 2 0 0 0-2-2z"/><circle cx="12" cy="12" r="3"/>') + '</button>' +
      '</aside><div class="es-pd-body">' +
      '<div class="es-pd-head"><h1>Elisee Scout — Dashboard Preparatore Atletico</h1>' +
      '<strong>Preparatore atletico: ' + esc(name.toUpperCase()) + '</strong></div>' +
      '<div class="es-pd-grid">' +

      '<section class="es-pd-card es-pd-indice"><h2>Profilo Preparatore Atletico</h2>' +
      ...
      '<div class="es-pd-metric"><span>Competenza tecnica</span><b>92%</b></div>' +
      '<div class="es-pd-metric"><span>Gestione carichi di lavoro</span><b>85%</b></div>' +
      '<div class="es-pd-metric"><span>Prevenzione infortuni</span><b>96%</b></div>' +
      '<div class="es-pd-metric"><span>Personalizzazione programmi</span><b>94%</b></div>' +
      '<div class="es-pd-metric"><span>Rapporto con staff medico</span><b>91%</b></div></section>' +

      '<section class="es-pd-card es-pd-radar">' +
      '<div class="es-pd-radar-tools"><span>Seleziona dati radar</span><span>Analisi attività di preparazione atletica</span><div class="es-pd-legend-pills"><span class="es-pd-pill-legend" style="color:#38bdf8"><i style="background:#38bdf8"></i> 2025 (Stagione Attuale)</span><span class="es-pd-pill-legend" style="color:#94a3b8"><i style="background:#64748b"></i> 2023 (Benchmark Storico)</span></div></div>' +
      radarSvg() + '</section>' +

      '<section class="es-pd-card es-pd-comply"><h2>Verifica &amp; Compliance staff tecnico</h2>' +
      '<div class="es-pd-ok"><span>Laurea Scienze Motorie</span><b>100%</b></div>' +
      '<div class="es-pd-ok"><span>Patentino UEFA Fitness</span><b>100%</b></div>' +
      '<div class="es-pd-ok"><span>Corso primo soccorso completo</span><b>100%</b></div>' +
      ...
      '<section class="es-pd-card es-pd-storico"><h2>Andamento preparazione fisica</h2>' +
      '<div class="es-pd-sparks">' +
      '<figure>' + spark([32, 40, 48, 55, 64, 74, 86], '#38bdf8') + '<figcaption>Carico di lavoro medio</figcaption></figure>' +
      '<figure>' + spark([70, 62, 55, 48, 42, 38, 34], '#f87171') + '<figcaption>Infortuni da sovraccarico</figcaption></figure>' +
      '<figure>' + spark([40, 48, 52, 58, 66, 74, 82], '#facc15') + '<figcaption>Livello di forma</figcaption></figure>' +
      '<figure>' + spark([48, 55, 60, 66, 74, 82, 90], '#4ade80') + '<figcaption>Stagione</figcaption></figure>' +
      '</div></section>' +

      '<section class="es-pd-card es-pd-mercato"><h2>Indice di efficacia atletica</h2>' +
      '<p class="es-at-grade">8,5/10 <small>+5,5%</small></p>' +
      '<div class="es-pd-mrow"><span>Valutazione allenatore capo</span><b>Ottima</b></div>' +
      '<div class="es-pd-mrow"><span>Trend condizione fisica</span><b>Crescente</b></div>' +
      '<div class="es-pd-mrow"><span>Richieste di conferma ruolo</span><b>Nessuna</b></div>' +
      ...
      '<section class="es-pd-card es-pd-registro"><h2>Registro sedute atletiche</h2>' +
      '<table class="es-pd-table"><thead><tr><th>Data</th><th>Tipo seduta</th><th>Intensità</th><th>Esito</th></tr></thead><tbody>' +
      '<tr><td>15/07/2026</td><td>Circuit training</td><td>Alta</td><td>Ottimo <i class="es-pd-dot g"></i></td></tr>' +
      '<tr><td>27/07/2026</td><td>Prevenzione</td><td>Media</td><td>Efficace <i class="es-pd-dot g"></i></td></tr>' +
      '<tr><td>16/09/2026</td><td>vs. Notaresco</td><td>Media</td><td>Efficace <i class="es-pd-dot g"></i></td></tr>' +
      '<tr><td>16/09/2026</td><td>vs. Chieti</td><td>Media</td><td>Efficace <i class="es-pd-dot g"></i></td></tr>' +
      '<tr><td>16/11/2026</td><td>Prevenzione</td><td>Media</td><td>Efficace <i class="es-pd-dot g"></i></td></tr>' +
      '<tr><td>16/09/2026</td><td>vs. Chieti</td><td>Media</td><td>In corso <i class="es-pd-dot y"></i></td></tr>' +
      '</tbody></table></section>' +

      '<section class="es-pd-card es-pd-trend"><h2>2023 vs 2024 vs 2025</h2>' +
      trendSvg() +
      '<button type="button" class="es-pd-edit" data-at="edit">Modifica anagrafica</button>' +
      '</section>' +
      '</div></div>';
  }
  ...
  function openAtEditModal(user) {
    ...
    backdrop.innerHTML = '<div class="es-edit-modal">' +
      ...
      '<div class="es-edit-field"><label>Nome</label><input id="es-at-nome" value="' + esc(user.nome || 'Eliseo') + '"></div>' +
      '<div class="es-edit-field"><label>Cognome</label><input id="es-at-cognome" value="' + esc(user.cognome || 'Miraglia') + '"></div>' +
      '<div class="es-edit-field"><label>Ruolo Ufficiale</label><input id="es-at-role" value="Preparatore Atletico" readonly></div>' +
      '<div class="es-edit-field"><label>Club / Organizzazione</label><input id="es-at-club" value="' + esc(user.squadra || user.club || 'Notaresco Calcio') + '"></div>' +
      '<div class="es-edit-field full"><label>Bio &amp; Note Operative</label><textarea id="es-at-bio" rows="3">' + esc(user.bio || 'Profilo accreditato e verificato su Elisee Scout per la stagione 2025/2026.') + '</textarea></div>' +
      '</div>' +
    ...
    backdrop.querySelector('.es-edit-btn-save').addEventListener('click', function () {
      ...
      if (typeof window.showToast === 'function') {
        window.showToast('Anagrafica Preparatore Atletico salvata con successo!', 'success');
      }
      render(user);
    });
  }
  ...
  function bind(host) {
    ...
    host.addEventListener('click', function (e) {
      var b = e.target.closest('[data-at]');
      if (!b) return;
