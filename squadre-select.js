...
(function () {
  ...
  var TEAMS = [];
  var LEAGUE_ORDER = [];
  var TEAMS_BY_GENDER_LEAGUE = { 'm': {}, 'f': {} };
  var LEAGUES_BY_GENDER = { 'm': [], 'f': [] };
  ...
  var TEAMS_FALLBACK = [
    {
      ...
      primary: '#0055d4',
      secondary: '#0b0f19',
      stadiumImage: 'immagini/stadi/city-foggia.jpg',
  ...
  var state = {
    ...
    leagueIndex: 0,
    index: 0,
    kit: 'home', // home | away | third | fourth | ...ordine cartella
    ready: false,
  ...
  function kitSlotsFor(team) {
    ...
    var isFgc = (team.id === 'foggia-city' || (team.name && String(team.name).toUpperCase().indexOf('FOGGIA') >= 0));
    if (isFgc) {
      delete team._cachedSlots;
      delete team.away;
      delete team.kitAway;
      delete team.third;
      delete team.kitThird;
      team.kitHome = 'immagini/kits-2d/foggia-city/home.png';
      team.kits = [{
        key: 'home',
        label: 'IN CASA',
        url: 'immagini/kits-2d/foggia-city/home.png',
    ...
    if (team._cachedSlots) return team._cachedSlots;
    var slots = [];
    var seenKey = {};
    var seenUrl = {};
  ...
  function ensureKitKey(team) {
    var isFgc = team && (team.id === 'foggia-city' || (team.name && String(team.name).toUpperCase().indexOf('FOGGIA') >= 0));
    if (isFgc) {
      state.kit = 'home';
    }
    var slots = kitSlotsFor(team);
  ...
  function applyCatalog(data) {
    ...
    for (var r = 0; r < REGISTERED_TEAMS.length; r++) {
      ...
      if (regTeam && regTeam.id && !seenIds[regTeam.id]) {
        if (regTeam.id === 'foggia-city' || (regTeam.name && String(regTeam.name).toUpperCase().indexOf('FOGGIA') >= 0)) {
          delete regTeam.away;
          delete regTeam.kitAway;
          delete regTeam.third;
          delete regTeam.kitThird;
          delete regTeam._cachedSlots;
          regTeam.kitHome = 'immagini/kits-2d/foggia-city/home.png';
          regTeam.kits = [{ key: 'home', label: 'IN CASA', url: 'immagini/kits-2d/foggia-city/home.png' }];
          regTeam.stadiumImage = 'immagini/stadi/city-foggia.jpg';
        }
    ...
    for (var gi = 0; gi < genders.length; gi++) {
      var gKey = genders[gi];
      var lgs = Object.keys(byGL[gKey]);
      for (var k = 0; k < lgs.length; k++) {
        var leagueName = lgs[k];
        byGL[gKey][leagueName].sort(function (a, b) {
          var an = String(a && a.name ? a.name : '').toUpperCase();
          var bn = String(b && b.name ? b.name : '').toUpperCase();
          return an < bn ? -1 : (an > bn ? 1 : 0);
        });
        counts[gKey][leagueName] = byGL[gKey][leagueName].length;
      }
      ...
      for (var m = 0; m < lgs.length; m++) {
        var remLg = lgs[m];
        if (!lgSeen[gKey][remLg] && byGL[gKey][remLg].length) {
          lgSeen[gKey][remLg] = true;
          lgOrder[gKey].push(remLg);
        }
      }
    }

    TEAMS_BY_GENDER_LEAGUE = byGL;
    LEAGUES_BY_GENDER = lgOrder;
    LEAGUE_TEAMS_COUNT = counts;
    CATALOG_READY = true;
    try {
      ...
      .then(function (data) {
        ...
        try {
          ...
          if (selSquadra && (selSquadra.id === 'foggia-city' || String(selSquadra.name).toUpperCase().indexOf('FOGGIA CITY') >= 0)) {
            ...
            selSquadra.kitHome = 'immagini/kits-2d/foggia-city/home.png';
            selSquadra.stadiumImage = 'immagini/stadi/city-foggia.jpg';
            delete selSquadra.away;
            delete selSquadra.kitAway;
            delete selSquadra.third;
            delete selSquadra.kitThird;
            delete selSquadra._cachedSlots;
            selSquadra.kits = [{ key: 'home', label: 'IN CASA', url: 'immagini/kits-2d/foggia-city/home.png' }];
  ...
  function loadCatalog() {
    ...
    // Fast sessionStorage check keyed by version
    var sessionKey = 'elisee_cat_fast_' + CATALOG_URL;
    try {
      var cachedRaw = sessionStorage.getItem(sessionKey);
      if (cachedRaw) {
        var cachedData = JSON.parse(cachedRaw);
        if (cachedData && Array.isArray(cachedData.teams) && cachedData.teams.length) {
          applyCatalog(cachedData);
          loadVerifiedList();
    ...
    return Promise.all([
      fetch(CATALOG_URL, { credentials: 'same-origin' }).then(function (r) {
      ...
      .then(function (pair) {
        var data = pair[0];
        if (!applyCatalog(data)) throw new Error('empty');
        try {
      ...
      .catch(function () {
        if (!TEAMS.length) {
          applyCatalog({ teams: TEAMS_FALLBACK.slice(), leagueOrder: [] });
        }
  ...
  function leaguesForGender() {
    return LEAGUES_BY_GENDER[state.gender] || [];
  }

  function currentLeague() {
    var leagues = LEAGUES_BY_GENDER[state.gender] || [];
  ...
  function filtered() {
    ...
    var g = state.gender;
    if (TEAMS_BY_GENDER_LEAGUE[g] && TEAMS_BY_GENDER_LEAGUE[g][lg]) {
      return TEAMS_BY_GENDER_LEAGUE[g][lg];
    }
    return [];
  }
  ...
  function updateStadiumPhoto(team) {
    ...
    if (!bg) return;
    var isFgc = team && (team.id === 'foggia-city' || (team.name && String(team.name).toUpperCase().indexOf('FOGGIA') >= 0) || (team.city && String(team.city).toUpperCase().indexOf('FOGGIA') >= 0));
    var src = team && team.stadiumImage ? String(team.stadiumImage).trim() : '';
    if (isFgc) {
      src = 'immagini/stadi/city-foggia.jpg';
    }
    if (!src) src = 'immagini/stadi/_default.jpg';
    var url = src.indexOf('?') >= 0 ? src : src + '?v=15';
    if (bg.dataset.currentBg === url) return;
    bg.dataset.currentBg = url;
    ...
    probe.onerror = function () {
      if (bg.dataset.currentBg !== url) return;
      bg.style.backgroundImage = 'url("immagini/stadi/_default.jpg?v=15")';
    };
    probe.src = url;
  }
  ...
  var _goldRingSynced = false;
  function syncGoldRing(force) {
    if (_goldRingSynced && !force) return;
  ...
  function leagueLogoPath(leagueName) {
    ...
    if (lg.indexOf('ECCELLENZA') === 0) {
      return 'immagini/squadre-loghi/eccellenza.png';
    }
  ...
  function leagueMatchesQuery(lg, q) {
    ...
    var list = (TEAMS_BY_GENDER_LEAGUE[state.gender] && TEAMS_BY_GENDER_LEAGUE[state.gender][lg]) || [];
  ...
  function buildLeaguePickerList(filter) {
    ...
    for (var i = 0; i < leagues.length; i++) {
      ...
      var nTeams = (LEAGUE_TEAMS_COUNT[state.gender] && LEAGUE_TEAMS_COUNT[state.gender][lg]) || 0;
  ...
  function stadiumSrc(team) {
    var isFgc = team && (team.id === 'foggia-city' || (team.name && String(team.name).toUpperCase().indexOf('FOGGIA') >= 0) || (team.city && String(team.city).toUpperCase().indexOf('FOGGIA') >= 0));
    if (isFgc) return 'immagini/stadi/city-foggia.jpg?v=15';
    var src = team && team.stadiumImage ? String(team.stadiumImage).trim() : '';
    if (!src) src = 'immagini/stadi/_default.jpg';
