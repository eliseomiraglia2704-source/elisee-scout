...
function userFromSupabase(info) {
  ...
  const nome = String(meta.given_name || bits[0] || (isPrivacy ? 'Manuel' : (isAdmin ? 'Alessandro' : 'Utente'))).trim();
  const cognome = String(meta.family_name || bits.slice(1).join(' ') || (isPrivacy ? 'Tucci' : (isAdmin ? 'Mancini' : 'Google'))).trim();
  return {
