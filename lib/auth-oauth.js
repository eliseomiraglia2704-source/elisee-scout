...
const GOOGLE_CLIENT_ID = process.env.GOOGLE_CLIENT_ID || '3785607635-c80g7kd5ini2n8a4rvnh3fg26krq5g.apps.googleusercontent.com';
...
async function startGoogleOAuth(req, res) {
  ...
  const redirectTo = originOf(req) + '/';
  const cookieVal = JSON.stringify({ verifier, returnTo, state, redirectTo });
  res.setHeader('Set-Cookie', pkceCookie(cookieVal, 600));
  ...
  const loc = SUPABASE_URL + '/auth/v1/authorize?' + q.toString();
  res.statusCode = 302;
...
function userFromSupabase(info) {
  ...
  const nome = String(meta.given_name || bits[0] || 'Utente').trim();
  const cognome = String(meta.family_name || bits.slice(1).join(' ') || 'Google').trim();
  return {
...
async function finishGoogleIdToken(idToken) {
  ...
  if (GOOGLE_CLIENT_ID && data.aud && data.aud !== GOOGLE_CLIENT_ID) {
    return { status: 401, body: { ok: false, error: 'google_aud_mismatch' } };
  }
  const user = {
    id: 'g_' + crypto.createHash('sha256').update(String(data.email).toLowerCase()).digest('hex').slice(0, 16),
    email: String(data.email).toLowerCase(),
    nome: String(data.given_name || 'Utente'),
    cognome: String(data.family_name || 'Google'),
    ruolo: 'Calciatore',
    provider: 'google'
  };
  return {
