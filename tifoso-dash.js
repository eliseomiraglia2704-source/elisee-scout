...
(function () {
  ...
  function html(user) {
    return window.EliseeDashReal.shell({
      user: user,
      title: 'Elisee Scout — Dashboard Tifoso',
      roleLabel: 'Tifoso',
      attr: 'tf',
      extraRail: '',
      radarTitle: 'Quadro passione',
      workTitle: 'Presenze',
      workEmpty: 'Nessuna presenza stadio registrata.',
      registroTitle: 'Registro gare',
      registroHeaders: ['Gara', 'Settore', 'Esito']
    });
  }
  ...
  function openTfEditModal(user) {
    ...
    backdrop.innerHTML = '<div class="es-edit-modal">' +
      ...
      '<div class="es-edit-field"><label>Nome</label><input id="es-tf-nome" value="' + esc(user.nome || '') + '"></div>' +
      '<div class="es-edit-field"><label>Cognome</label><input id="es-tf-cognome" value="' + esc(user.cognome || '') + '"></div>' +
      '<div class="es-edit-field"><label>Ruolo Ufficiale</label><input id="es-tf-role" value="Tifoso / Spettatore" readonly></div>' +
      '<div class="es-edit-field"><label>Club / Organizzazione</label><input id="es-tf-club" value="' + esc(user.squadra || user.club || '') + '"></div>' +
      '<div class="es-edit-field full"><label>Bio &amp; Note Operative</label><textarea id="es-tf-bio" rows="3">' + esc(user.bio || '') + '</textarea></div>' +
      '</div>' +
