...
Ultimo fatto: **Dashboard Calciatore v3.5: Report Tecnico Professionale, Tutela Minori, Radar a 3 Serie & Profilo Attivo Autogestito**: 1. **Rimozione totale emoji**: sostituite tutte le emoji sui pulsanti azione e nei titoli con icone lineari SVG monocolore standard (stroke 1.75px); 2. **Tutela Minori & Reinquadramento Mercato**: rinominata la card in *"Interesse Scouting & Percorso Sportivo"*, rimossa qualsiasi quotazione economica o concetto di rivendita per atleti minorenni (`< 18` anni) con focus esclusivo su formazione e salvaguardia FIGC; per maggiorenni introdotta *"Valutazione Tecnica Indicativa di Categoria"* con fonte trasparente di calcolo; 3. **Radar Prestazioni a 3 Serie**: aggiunta la serie di riferimento *"Media Girone / Categoria"* a confronto con *"Stagione Attuale 2025/26"* e *"Benchmark Storico 2023/24"*, con interattività completa su ogni metrica (apertura modale con clip video di Match Analysis e contesto gara); 4. **Storico Infortuni & Disponibilità Fisica**: nuova card sanitaria con tracciamento giorni di recupero, continuità fisica (94%) e indicatore di accesso riservato; 5. **Card "Il Mio Profilo" (Dati Dichiarati Atleta)**: autocompilazione attiva con Bio, Obiettivi di Carriera, Ruoli secondari, Toggle provini/trasferimento, Stato contrattuale e Autovalutazione mensile; 6. **Principio di Trasparenza Fonti**: distinzione visiva chiara con badge B2B per dati IA, dati Club/Staff e dati dichiarati dall'atleta; 7. **Reinquadramento Percorso di Crescita**: rinominato il simulatore in *"Percorso di Crescita Proiettato (IA Career Projection)"*. File: `player-dash.js`, `player-dash.css`, `role-actions-runtime.js`, `index.html`, `sw.js`. Cache `PLAYER_DASH_PRO1`.
Feature precedente: Sicurezza & Autenticazione: Risoluzione Backdoor OTP e Token Gate Serverless Vercel per Master Admin.
