...
Ultimo fatto: **Simulatore Ruoli Creatore: Risoluzione Blocco Cambio Ruolo & Sincronizzazione Dinamica Dashboard**: 1. **Risolto lock ruolo in `player-profile.js`**: rimosso il meccanismo per cui `applyStaffIdentity` recuperava la vecchia identità staff salvata in memoria (`elisee_role_identity`) e sovrascriveva forzatamente il ruolo a "Allenatore" quando l'utente selezionava "Calciatore", "Tifoso" o "Società"; 2. **Parsing Robusto Token Admin**: eliminata la chiamata a `decodeURIComponent(escape())` nel client che poteva fallire e bloccare il click sui bottoni ruolo; 3. **Aggiornamento & Dispatch Eventi Globali**: al click su qualsiasi card ruolo nel simulatore, il sistema aggiorna istantaneamente storage, classi visive di `#user-dossier-view-group` e scatena `elisee:role-changed` ed `elisee:view-changed` per renderizzare subito la dashboard corretta. File: `player-profile.js`, `creator-role-switcher.js`, `index.html`, `sw.js`, `CONTINUA_DA_QUI.md`. Cache `ROLE_SWITCH_FIX1`.
Feature precedente: Sicurezza & Autenticazione: Fix Tolleranza Case-Insensitive Password Master Admin & Toggle Visivo Password.
