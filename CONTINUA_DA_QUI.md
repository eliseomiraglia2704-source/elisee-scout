...
Ultimo fatto: **Selettore Stagione & Sistema di Valutazione Pubblica Multi-Ruolo Strutturata (public-rating-system.js)**: 1. **Cruscotto Selettore Stagione**: implementato in alto a destra nella testata analitica con selettore a tendina e frecce `◀` `▶` (stagioni `2026/27`, `2025/26`, `2024/25`, `2023/24`, `2022/23`); 2. **Ricalcolo Integrale al Cambio Stagione**: aggiornamento dinamico immediato di radar prestazioni a 12 assi (con benchmark relativo alla stagione selezionata), statistiche di dettaglio, metriche e tabella registro match; stato esplicito di avviso ("Nessun dato registrato per la stagione selezionata") per stagioni prive di storico; 3. **Sistema di Valutazione Pubblica Multi-Ruolo (`public-rating-system.js`)**: valutazioni strutturate su 4 criteri specifici per ruolo (scale 1-5 su rendimento, continuità, professionalità, impatto tattico, gestione gruppo, ecc.), con esclusione categorica del testo libero pubblico per prevenzione diffamazione e tutela legale; 4. **Tutela Minori Assoluta (Under 18)**: valutazione pubblica disabilitata di default per profili minorenni con banner esplicito di conformità FIGC/GDPR; 5. **Anti-Abuso & Trasparenza B2B**: autenticazione obbligatoria, 1 voto per utente per soggetto con possibilità di aggiornamento, trasparenza sul conteggio totale delle valutazioni certificate; 6. **Distinzione Netta Fonti**: badge `[PERCEZIONE PUBBLICA & COMMUNITY]`; 7. **Collegamento Art. 22 GDPR**: modale per richiesta di revisione statistica e intervento umano in caso di sospetto afflusso di voti anomali. File: `public-rating-system.js`, `player-dash.js`, `index.html`, `sw.js`, `CONTINUA_DA_QUI.md`. Cache `SEASON_RATING_SYS1`.
Feature precedente: Dashboard Calciatore v3.5: Report Tecnico Professionale, Tutela Minori, Radar a 3 Serie & Profilo Attivo Autogestito.
