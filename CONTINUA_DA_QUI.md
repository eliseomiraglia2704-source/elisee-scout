...
Ultimo fatto: **Sicurezza & Autenticazione: Risoluzione Backdoor OTP e Token Gate Serverless Vercel per Master Admin**: 1. **Eliminazione totale della backdoor OTP**: rimossi definitivamente i codici hardcoded `0000`/`1234`, la pillola a schermo che mostrava il codice e la simulazione insicura in `sessionStorage`; 2. **Endpoint Serverless `api/auth-otp.js`**: generazione crittografica OTP 4 cifre lato server, rate limiting su invii (max 3 per 10 min) e su verifiche (blocco dopo 5 tentativi errati), scadenza 10 minuti con hash HMAC-SHA256; 3. **Endpoint Serverless `api/auth-admin.js`**: protezione reale del Simulatore Ruoli Creatore e delle funzioni Admin con Master Secret (`ADMIN_SECRET` su Vercel), rate limiting per IP (max 5 tentativi falliti per 15 minuti) e rilascio di token crittografico firmato HMAC-SHA256 valido per 2 ore; 4. **Frontend `creator-role-switcher.js`**: modale per l'inserimento del PIN Master che richiede e convalida il token serverless prima di consentire l'accesso, salvando il token firmato unicamente in `sessionStorage` temporaneo (azzerato alla chiusura della sessione); 5. **Compatibilità Server Locale `elisee_up.py`**: implementati i gestori Python `/api/auth-otp` e `/api/auth-admin` per parità funzionale completa tra sviluppo locale e produzione Vercel. File: `api/auth-otp.js`, `api/auth-admin.js`, `verifica-account.js`, `creator-role-switcher.js`, `elisee_up.py`, `index.html`, `sw.js`. Cache `OTP_ADMIN_SECURE1`.
Feature precedente: Scadenziario Federale: Rifinitura Finale verso Stile B2B / Gestionale Professionale.
