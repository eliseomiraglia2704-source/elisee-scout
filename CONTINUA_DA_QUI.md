...
Ultimo fatto: **Scadenzario Federale: Roll-over Stagionale, Distinzione Voci Ricorrenti/Una Tantum & Archivio Storico Certificato**: 1. Associazione esplicita di ogni adempimento federale a una specifica annata (`season: '2026/27'`) e flag di ricorrenza (`recurrence: 'seasonal'` vs `'one_off'`); 2. Implementata la funzione `advanceFootballSeason(data, targetSeason)` e la modale di conferma controllata `openNewSeasonConfirmationModal(data)` per avviare la nuova stagione (es. `2027/28`): le voci ricorrenti (iscrizioni, fideiussioni, visite mediche, ecc.) vengono duplicate in stato *"Da completare"* con date da impostare dai nuovi C.U., mentre le voci una tantum (MOG, Tutela Minori, Statuto RAS) rimangono valide a tempo indeterminato senza duplicazioni inutili; 3. Implementato il selettore vista storico `#sel-deadlines-season-view` in `openDeadlinesManagerModal`: permette di consultare l'Archivio Storico Certificato in sola lettura per attestazione di conformità e audit federale di ogni annata trascorsa, senza mai sovrascrivere lo storico; 4. Aggiunto l'indicatore di avvicinamento cambio stagione nei mesi di transizione (maggio/giugno/luglio) sia nella modale che nella card della dashboard overview. File: `pres-dash.js`, `index.html`, `sw.js`. Cache `SEASON_ROLLOVER1`.
Feature precedente: Scadenzario Federale: Stato "Completato", Data Reale di Assolvimento & Rimozione Duplicazione Fideiussione.
