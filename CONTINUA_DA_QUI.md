...
Ultimo fatto: **Fix navigazione tasto indietro browser nelle schermate di dettaglio (popstate / sub-routes)**: Risolto il problema per cui cliccando la freccia indietro del browser da una schermata di dettaglio (es. *Centro Sportivo & Staff Tecnico*, *Stadio*, *Statistiche*, *Sponsor*, *Classifica*, *Calendario*) si veniva reindirizzati alla Bacheca Reclutamento (`#bacheca-annunci`): 1. Implementata la sincronizzazione della History API con `history.pushState` (`#user-dossier-portal?sub=...`) per tutte le 6 sub-view in `pres-dash.js`; 2. Aggiunto listener `popstate` che ripristina la vista principale panoramica (`currentView = 'overview'`) senza abbandonare l'area riservata; 3. Aggiunto pulsante top `← Torna alla Panoramica` in ciascun header di dettaglio e configurati tutti i pulsanti `data-action="go-back"` con `window.history.back()`; 4. Aggiornato `applyViewFromBrowserHistory` in `app.js` per supportare sub-query per `user-dossier`. File: `pres-dash.js`, `app.js`, `index.html`, `sw.js`. Cache `NAV1`.
Feature precedente: Simulatore Ruoli Creatore (ADMIN ONLY) & Restyling Enterprise SaaS.
